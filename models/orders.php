 ...
 * @since       1.6
 */
class CrmModelOrders extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*');
		$query->from($db->quoteName('#__crm_order') . ' AS a');
		...
		// Join over
		$query->select('c.name AS customer_name')
			->join('LEFT', $db->quoteName('#__crm_customer') . ' AS c ON c.id = a.customer_id');
